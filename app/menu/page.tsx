    ...
    <div className="min-h-screen">
      ...
      <main className="py-[4.5rem]">
        <div className="w-[min(calc(100%-1.5rem),76rem)] md:w-[min(calc(100%-3rem),76rem)] mx-auto">
          ...
          />
          <div className="grid gap-4 rounded-[1.75rem] p-[1.4rem] mb-6 border border-line bg-[rgba(255,255,255,0.72)] [box-shadow:0_20px_60px_rgba(49,40,33,0.08)]">
            <div>
                  ...
                  {collection.items.map((item) => (
                    <li key={item.name} className="grid gap-[0.35rem]">
                      <div className="flex items-center justify-between gap-3 font-bold">
                        <span>{item.name}</span>
                        {item.tag ? (
                          <span className="px-[0.55rem] py-[0.28rem] rounded-full bg-surface-muted text-text-soft text-[0.72rem]">
                            {item.tag}
                          </span>
                        ) : null}
                      </div>
                      <p className="m-0 text-text-soft leading-[1.7]">
                        {item.description}
                      </p>
                    </li>
                  ))}
